...
class SalesforceMetadataParser:
    ...
    def parse(self) -> MetadataSnapshot:
        ...
        for package_root in package_roots:
            ...
            objects.update(self._parse_objects(package_root))
            profiles.extend(self._parse_security_folder(package_root / "profiles", "profile"))
            permission_sets.extend(
                self._parse_security_folder(package_root / "permissionsets", "permission_set")
            )
            apex_artifacts.extend(self._parse_apex_folder(package_root / "classes", "class"))
        ...
        for object_dir in sorted(path for path in objects_dir.iterdir() if path.is_dir()):
            ...
            if validation_rules_dir.exists():
                for validation_rule_file in sorted(validation_rules_dir.glob("*.validationRule-meta.xml")):
                    info.validation_rules.append(self._parse_validation_rule(validation_rule_file))
